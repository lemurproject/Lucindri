 ...
 * sub {@link Scorer}.
 */
class IndriScoreIfScorer extends Scorer {
	 ...
	 */
	public IndriScoreIfScorer(Scorer reqScorer, Scorer exclScorer) {
	...
	/**
	 * Confirms whether or not the given {@link TwoPhaseIterator} matches on the
	 * current document.
	 */
	...
	/**
	 * Estimation of the number of operations required to call DISI.advance. This is
	 * likely completely wrong, especially given that the cost of this method
	 * usually depends on how far you want to advance, but it's probably better than
	 * nothing.
	 */
	private static final int ADVANCE_COST = 10;

	private static float matchCost(DocIdSetIterator reqApproximation, TwoPhaseIterator reqTwoPhaseIterator,
			DocIdSetIterator exclApproximation, TwoPhaseIterator exclTwoPhaseIterator) {
	...
	public TwoPhaseIterator twoPhaseIterator() {
		final float matchCost = matchCost(reqApproximation, reqTwoPhaseIterator, exclApproximation,
				exclTwoPhaseIterator);

		if (reqTwoPhaseIterator == null || (exclTwoPhaseIterator != null
